...
function _hydro_postexec --on-event fish_postexec
    ...
    set --local error_color (set_color $fish_color_error)
    set --global _hydro_status "$_hydro_newline$normal$hydro_symbol_prompt"

    for code in $last_status
        if test $code -ne 0
            set --global _hydro_status "$error_color| "(echo $last_status)" $_hydro_newline$error_color$hydro_symbol_prompt"
            break
        end
    end
end
...
function _hydro_prompt --on-event fish_prompt
    set --query _hydro_status || set --global _hydro_status "$_hydro_newline$hydro_symbol_prompt"
